...
import { ChevronDown, ChevronRight, FileText, LayoutGrid, Plus, Play, Trash2, FileBarChart } from "lucide-react";
                        ...
                        <div className="space-y-2">
                          {items.map((report) => (
                            <div
                              ...
                              className="px-3 py-2 rounded-lg border border-gray-100 bg-white hover:border-blue-200 hover:bg-gray-50 transition-colors group cursor-pointer"
                            >
                              ...
                              <div className="flex items-center gap-1 mt-1 opacity-0 group-hover:opacity-100 transition-opacity">
                                <span className="p-1 rounded text-gray-400 group-hover:text-gray-600" title="Use"><Play className="w-3 h-3" /></span>
                                <button onClick={(e) => { e.stopPropagation(); handleDelete(report); }} className="p-1 rounded hover:bg-red-100 text-gray-400 hover:text-red-500" title="Delete"><Trash2 className="w-3 h-3" /></button>
                              </div>
                            </div>
                          ))}
                        </div>
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-3 gap-3">
                  {filteredReports.map((report) => (
                    <div
                      key={report.id}
                      onClick={() => handleEditReport(report)}
                      className="flex flex-col gap-2 p-3 rounded-xl border border-gray-100 bg-white hover:border-blue-200 hover:bg-gray-50 transition-colors group cursor-pointer"
                    >
                      <div className="flex items-center justify-between">
                        <FileBarChart className="w-5 h-5 text-blue-400" />
                        <div className="flex items-center gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                          <span className="p-1 rounded text-gray-400 group-hover:text-gray-600" title="Use"><Play className="w-3.5 h-3.5" /></span>
                          <button onClick={(e) => { e.stopPropagation(); handleDelete(report); }} className="p-1 rounded hover:bg-red-100 text-gray-400 hover:text-red-500" title="Delete"><Trash2 className="w-3.5 h-3.5" /></button>
                        </div>
                      </div>
                      <p className="text-sm font-medium text-gray-800 truncate">{report.name}</p>
                ...
                <div className="space-y-1.5">
                  {filteredReports.map((report) => (
                    <div
                      key={report.id}
                      onClick={() => handleEditReport(report)}
                      className="flex items-center gap-2 px-3 py-2 rounded-lg border border-gray-100 bg-white hover:border-blue-200 hover:bg-gray-50 transition-colors group cursor-pointer"
                    >
                      ...
                      <div className="flex items-center gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                        <span
                          className="p-1 rounded text-gray-400 group-hover:text-gray-600"
                          title="Use"
                        >
