...
export function buildScoringBlocksFromTemplate(template) {
  return (template.scoring_blocks || []).map((block) => ({
    ...
    name: block.name,
    weight: block.weight,
    multiplier_enabled: block.multiplier_enabled || false,
    multiplier: block.multiplier == null ? 1 : block.multiplier,
    criteria: (block.criteria || []).map((crit) => ({
      id: crit.id,
      number: crit.number,
      name: crit.name,
      category: crit.category,
      multiplier_enabled: crit.multiplier_enabled || false,
      multiplier: crit.multiplier == null ? 1 : crit.multiplier,
      primary_score: null,
      primary_notes: "",
