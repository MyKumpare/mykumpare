...
export default function RescoreConfirmDialog({ priorScore, onCreated, onClose }) {
  ...
  const createRescoreMutation = useMutation({
    mutationFn: async () => {
      // Deep-copy the prior scoring_blocks, preserving structure + notes as baseline
      // but resetting all workflow flags on each criterion
      const copiedBlocks = (priorScore.scoring_blocks || []).map((block) => ({
        ...
        criteria: (block.criteria || []).map((crit) => ({
          ...
          // so the analyst can adjust from the last known position
      ...
      const newScore = await base44.entities.ScoringMatrixScore.create({
        ...
        // Do NOT carry over secondary analyst — re-scoring starts fresh
