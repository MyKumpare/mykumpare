...
const STORAGE_KEY = "utility_layout_v1";
...
function load(modules, defaultCategories) {
  try {
...
/**
 * Per-user Utility layout: an ordered list of categories, each holding an
 ...
 * `modules` is the role-filtered active module list; `defaultCategories` is
 * the role-appropriate default layout used on first load.
 */
export function useUtilityLayout(modules, defaultCategories) {
  const [categories, setCategories] = useState(() => load(modules, defaultCategories));

  // Re-reconcile when the active module set changes (e.g. role change) so
  // newly-eligible modules appear and removed ones drop out.
  useEffect(() => {
