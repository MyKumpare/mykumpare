...
export default function UtilityFullScreenModal({ open, onClose, deletedCount = 0, onFirmClick, defaultView, onActivityClick, onContactClick, onProductClick, firms = [], products = [], activeContacts = [] }) {
      ...
      <DialogContent className="max-w-none w-screen h-screen sm:rounded-none p-0 gap-0 flex flex-col overflow-hidden [&>button]:!hidden">
        ...
        <div className="flex-1 overflow-y-auto bg-gray-50/80 px-4 sm:px-6 py-4">
          <div className="max-w-3xl mx-auto">
            <UtilitySection key={forceKey} deletedCount={deletedCount} forceExpanded={true} onFirmClick={onFirmClick} defaultView={defaultView} onActivityClick={onActivityClick} onContactClick={onContactClick} onProductClick={onProductClick} firms={firms} products={products} activeContacts={activeContacts} />
          </div>
        </div>
      </DialogContent>
    </Dialog>
