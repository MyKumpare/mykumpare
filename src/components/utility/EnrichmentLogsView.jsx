...
export default function EnrichmentLogsView() {
  ...
  if (logs.length === 0) {
    return (
      <div className="flex flex-col items-center justify-center py-10 text-gray-400">
        ...
        <p className="text-xs">No enrichment attempts yet.</p>
      </div>
    );
  }
        ...
        return (
          ...
          >
            ...
            >
              ...
              {log.status === "success" && <CheckCircle2 className="w-3.5 h-3.5 text-green-500 flex-shrink-0" />}
              {log.status === "error" && <XCircle className="w-3.5 h-3.5 text-red-500 flex-shrink-0" />}
              {log.status === "no_data" && <AlertCircle className="w-3.5 h-3.5 text-amber-500 flex-shrink-0" />}
              ...
              <div className="flex items-center gap-2 flex-shrink-0">
                {log.total_people_found > 0 && (
                  <>
                    <span className="inline-flex items-center gap-0.5 text-[11px] text-green-600" title="New contacts">
                      <UserPlus className="w-3 h-3" /> {log.people_new}
                    </span>
                    {log.people_skipped > 0 && (
                      <span className="inline-flex items-center gap-0.5 text-[11px] text-gray-400" title="Skipped (already exists)">
                        <UserMinus className="w-3 h-3" /> {log.people_skipped}
                      </span>
                    )}
                    {log.people_similar > 0 && (
                      <span className="inline-flex items-center gap-0.5 text-[11px] text-amber-600" title="Similar — needs review">
              ...
              <div className="px-3 pb-3 space-y-2 border-t border-gray-100">
                ...
                {(log.fields_new > 0 || log.fields_skipped > 0 || log.fields_similar > 0) && (
                  <div className="flex items-center gap-3 text-[11px] text-gray-500">
                    <span className="text-[10px] font-semibold text-gray-400 uppercase tracking-wide">Firm fields:</span>
