...
/**
 * Fetches product records using server-side cursor-based pagination under the
 ...
 *
 * Two modes:
 *  - Batch mode (body has `cursor`/`limit`/`batchMode`): returns one page { records, nextCursor, hasMore }
 *  - Legacy mode (no body or body without those keys): returns ALL records { records, total, batches }
 */
...
export default async function(req: Request): Promise<Response> {
  try {
    ...
    if (batchMode) {
      ...
      const batch = await fetchWithRetry(entities.Product, filter, '-created_date', limit);
    ...
    while (true) {
      ...
      for (let outerRetry = 0; outerRetry < 3; outerRetry++) {
        try {
          batch = await fetchWithRetry(entities.Product, filter, '-created_date', BATCH);
          break;
        } catch (err: any) {
